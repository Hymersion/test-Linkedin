...
const generateCommentSuggestions = async (target, posts, objectives) => {
    ...
    if (!candidatePosts.length) {
        const base = (target && (target.headline || target.fullName))
            ? `${target.fullName || "Ce profil"} — ${target.headline || ""}`.trim()
            : "profil suivi";
        return [{
            id: `${target.profileUrl || 'profile'}-${Date.now()}-fallback-empty`,
            urn: "",
            postText: `Aucun post détecté automatiquement pour ${base}.`,
            comment: `Bonjour ${target.fullName || ""}, votre positionnement sur ${target.headline || "votre expertise"} est intéressant. ${objectives ? `Je cherche surtout: ${objectives}.` : "Je serais ravi d'échanger sur vos sujets récents."}`.trim(),
            reason: "Fallback généré car aucun post exploitable n'a été détecté.",
      ...
      (async () => {
          ...
          try {
              ...
              for (const target of payload) {
                  ...
                  const hasPosts = scanResult && scanResult.success && Array.isArray(scanResult.posts);
                  const posts = hasPosts ? scanResult.posts : [];
                  if (hasPosts) {
                      totalPosts += posts.length;
                  }
                  const commentSuggestions = await generateCommentSuggestions(target, posts, request.objectives || "");
                  if (!hasPosts) {
                      const fallbackReason = scanResult && scanResult.error
                          ? scanResult.error
                          : "Aucun post récupéré pendant le scan (contenu non chargé ou inaccessible).";
                      if (commentSuggestions[0]) {
                          commentSuggestions[0].reason = `${commentSuggestions[0].reason || ""} ${fallbackReason}`.trim();
                      }
                  }
                  ...
                  suggestions.push({
                      ...
                      scannedAt: Date.now(),
                      scanError: hasPosts ? "" : (scanResult && scanResult.error ? scanResult.error : "scan_empty")
                  });
